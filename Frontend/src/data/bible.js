// Bible chapter and verse counts for reference selection only.
// Derived from https://github.com/oflannabhra/bible.json (no verse text included).
// Note: chapter/verse numbering can vary slightly by translation.
...
export const BIBLE_BOOKS = [
  ...
  {
    id: "lamentations",
    name: { es: "Lamentaciones", en: "Lamentations" },
    chapters: [22, 22, 66, 22, 22],
  },
  {
    id: "ezekiel",
    name: { es: "Ezequiel", en: "Ezekiel" },
...
const BIBLE_BOOKS_BY_ID = Object.fromEntries(BIBLE_BOOKS.map((book) => [book.id, book]));

export const DEFAULT_BIBLE_SELECTION = {
  bookId: "philippians",
  chapter: 4,
  verse: 6,
};
...
export function getBibleBook(bookId) {
  return BIBLE_BOOKS_BY_ID[String(bookId || "").trim()] || BIBLE_BOOKS[0];
}

export function getBibleChapterOptions(bookId) {
  const book = getBibleBook(bookId);
  return Array.from({ length: book?.chapters?.length || 0 }, (_, index) => index + 1);
}

export function getBibleVerseOptions(bookId, chapterNumber) {
  const book = getBibleBook(bookId);
  const safeChapter = Math.max(1, Number(chapterNumber) || 1);
  const totalVerses = Number(book?.chapters?.[safeChapter - 1]) || 0;
  return Array.from({ length: totalVerses }, (_, index) => index + 1);
}

export function formatBibleReference(selection, language = "es") {
  const book = getBibleBook(selection?.bookId);
  const chapter = Math.max(1, Number(selection?.chapter) || 1);
  const verse = Math.max(1, Number(selection?.verse) || 1);
  const name = book?.name?.[language] || book?.name?.es || book?.name?.en || "";
  if (!name) return "";
