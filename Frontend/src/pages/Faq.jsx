...
import { normalizeFaqConfig } from "../utils/faqs";

export default function Faq({ faqConfig, t, language, socialConfig }) {
  const items = normalizeFaqConfig(faqConfig).items
    .map((item) => ({
      id: item.id,
      q: l10n(item.question, language),
      a: l10n(item.answer, language),
      enabled: item.enabled,
    }))
    .filter((item) => item.enabled && item.q && item.a);
